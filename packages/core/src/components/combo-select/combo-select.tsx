...
export class ComboSelect {
   ...
   * A reference to the popover in the single select.
   */
  private targetRef: HTMLBrInputElement | null;
  /**
   * A reference to the popover in the single select.
   */
   ...
   * @visibility hidden
   */
  @Prop({ reflect: true }) readonly internalId: string = `br-combo-select-${comboSelectId++}`;
  /**
  ...
  connectedCallback() {
    ...
    if (this.targetRef) {
      ...
      this.targetRef.addEventListener('input', this.preventInputEvents);
    }
  }
  ...
  componentWillLoad() {
    ...
    if (hasTarget?.tagName.toLowerCase() !== 'br-input' && DebugMode.currentDebug && hasDebugger) {
      console.error(`ERROR - The target element must be an input.`, this.elm);
    }
    ...
    this.targetRef = hasTarget as HTMLBrInputElement;
  ...
  disconnectedCallback() {
    ...
    if (this.targetRef) {
      ...
      this.targetRef.removeEventListener('input', this.preventInputEvents);
    }
    if (this.listRef) {
  ...
  private handleListValueChange = (e: CustomEvent<{ value: any[] | undefined }>) => {
    ...
    if (!isEqual(this.value, e.detail.value)) {
      ...
      if (this.selectValueIconRef) {
        this.selectValueIconRef.value = itemValue;
      }
  ...
  private handleInputValueChange = (e: CustomEvent) => {
    ...
    const itemsToUse = items ? Array.from(items) : [];
    const item = itemValue ? itemsToUse.find((it) => isEqual(it.value, itemValue)) : undefined;
    const label = item?.label || item?.textContent || (this.value && this.value[0]) || undefined;
    if (label !== value) {
      this.filter = value;
    }
  ...
  handleKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'Enter' && this.filter) {
      ...
      const label = item?.label || item?.textContent || undefined;
      if (this.targetRef) {
        return (this.targetRef.value = label);
      }
    }
    if (e.key === 'Enter' && !this.filter) {
