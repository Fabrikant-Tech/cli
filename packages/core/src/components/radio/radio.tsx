...
export class Radio {
   ...
   */
  @Element() elm: HTMLBrRadioElement;
  /**
  ...
  private handleChange(event: globalThis.Event) {
    ...
    this.checked = (event.target as globalThis.HTMLInputElement).checked;
    const valueToSet = this.getValueFromProps();
    this.internals?.setFormValue(valueToSet !== '' ? valueToSet : null);
  }
  ...
  private handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'ArrowUp') {
      if (
        ...
        previousElementSibling.tagName.toLowerCase() === 'br-radio' &&
        (previousElementSibling as HTMLBrRadioElement).name === this.name
      ) {
        (this.elm.previousElementSibling as HTMLBrRadioElement).shadowRoot
          ?.querySelector('input')
          ?.click();
        (this.elm.previousElementSibling as HTMLBrRadioElement).focus({ preventScroll: true });
      }
    }
    if (e.key === 'ArrowDown') {
      if (
        nextElementSibling &&
        nextElementSibling.tagName.toLowerCase() === 'br-radio' &&
        (nextElementSibling as HTMLBrRadioElement).name === this.name
      ) {
        (this.elm.nextElementSibling as HTMLBrRadioElement).shadowRoot
          ?.querySelector('input')
          ?.click();
        (this.elm.nextElementSibling as HTMLBrRadioElement).focus({ preventScroll: true });
      }
    }
  };
  ...
  render() {
    ...
    return (
      <Host aria-checked={this.checked ? 'true' : undefined} role="radio">
        <div
          class={{
            'br-radio-wrapper': true,
            'br-radio-was-touched': isTouched || this.active === true,
            'br-radio-focused': this.focused || this.active === true,
            'br-radio-disabled': this.disabled,
            'br-radio-invalid': isTouched && !this.internalValidityState.valid,
          }}
        >
          <div
            class="br-radio-content"
