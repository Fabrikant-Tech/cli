# Dialog header



<!-- Auto Generated Below -->


## Overview

The Dialog Content component wraps around the content of dialogs to serve as a stable root for conditional rendering.

## Properties

| Property          | Attribute           | Description                                      | Type                | Default        |
| ----------------- | ------------------- | ------------------------------------------------ | ------------------- | -------------- |
| `showCloseButton` | `show-close-button` | Determines if the close affordance is displayed. | `boolean`           | `true`         |
| `theme`           | `theme`             | Defines the theme of the component.              | `"Dark" \| "Light"` | `ThemeDefault` |


## Slots

| Slot        | Description                                    |
| ----------- | ---------------------------------------------- |
|             | Passes content to the Dialog Content.          |
| `"content"` | Passes advanced content to the Drawer Content. |


----------------------------------------------


