 ...
 * @category Layout
 * @slot header - Passes the sidebar content.
 * @slot content - Passes the content.
 */
@Component({
  tag: 'br-layout-header-content',
...
export class LayoutSidebarContent implements ComponentInterface {
  ...
  /**
   * Determines what type of scrolling is allowed in the header part of the component.
   * @category Appearance
   */
  @Prop() headerAllowedScroll: 'horizontal' | 'vertical' | 'any' | false = false;
  /**
  ...
  /**
   * Determines the direction of the header.
   * @category Appearance
   * @visibility persistent
   * @order 1
   */
  @Prop() direction: 'top' | 'bottom' = 'top';
  /**
    ...
    return (
      ...
      >
        <br-container
          ...
          fullHeight={true}
          theme={this.theme}
          direction={this.direction === 'top' ? 'column' : 'column-reverse'}
        >
          <br-container
            direction="column"
            width="100%"
            shrink={false}
            theme={this.theme}
            allowedScroll={this.headerAllowedScroll}
            zIndex={2}
